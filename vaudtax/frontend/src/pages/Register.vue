  ...
  <div class="min-vh-100 d-flex align-items-center justify-content-center bg-page position-relative overflow-hidden">
    ...
    <div class="card card-brand shadow-lg p-4 p-md-5 position-relative z-1" style="width:100%;max-width:450px">
      <div class="text-center mb-4">
        ...
        <h2 class="fw-bold text-slate">{{ t('auth.register_title') }}</h2>
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="mb-3">
          ...
          <input v-model="email" type="email" required class="form-control" placeholder="exemple@email.com" />
        </div>
        <div class="mb-4">
      ...
      <p class="mt-4 text-center text-muted-brand" style="font-size:.875rem">
        {{ t('auth.has_account') }}
        <router-link to="/login" class="text-brand fw-bold text-decoration-none">{{ t('auth.link_login') }}</router-link>
      </p>
    </div>
  </div>
</template>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.username?.[0] || err.message || 'Une erreur est survenue'
  } finally {
    loading.value = false
  }
}
